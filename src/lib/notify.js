...
//   LEAD_NOTIFY_FROM         = "Zafah <onboarding@resend.dev>" (a verified sender)

const FROM = () => process.env.LEAD_NOTIFY_FROM || "Zafah <onboarding@resend.dev>";
...
export function notifyNewLead(lead) {
  const html = `
    ...
    <p style="font-family:sans-serif">A new lead came in via Zafah (الزفة):</p>
...
export function notifyNewVenueSubmission(venue) {
  const html = `
    ...
    <p style="font-family:sans-serif">Review it in the Zafah admin dashboard before it goes public.</p>`;
  return sendEmail({
    to: adminRecipients(),
...
export function notifyContactMessage(msg) {
  const html = `
    <h2 style="font-family:sans-serif">New contact message — Zafah (الزفة)</h2>
    <table style="font-family:sans-serif;border-collapse:collapse" cellpadding="6">
...
export function notifyOwnerVenueReceived(venue) {
  ...
  const html = `
    <h2 style="font-family:sans-serif">We received your venue — Zafah (الزفة)</h2>
  ...
  return sendEmail({ to: venue.owner_email, subject: "We received your venue — Zafah", html, context: "owner confirmation" });
}

export function notifyCoupleInquiryReceived(lead) {
  if (!lead?.email) return Promise.resolve({ skipped: true });
  const html = `
    <h2 style="font-family:sans-serif">Your inquiry was sent — Zafah (الزفة)</h2>
  ...
  return sendEmail({ to: lead.email, subject: "Your inquiry was sent — Zafah", html, context: "couple confirmation" });
}
